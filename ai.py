...
heuristic = [[0 for _ in range(5)] for _ in range(5)]
...
def BasicA(maze, start, obstacles):
    ...
    while not frontier.empty():
        ...
        #     obstacles[curr_loc[0]][curr_loc[1]] = 'w'
        #     # create.printMaze(obstacles, len(maze), len(maze[0]))
        #     break
        ...
        if curr_loc[0] > 1 and (up not in close_list) and obstacles[up[0]][up[1]] != 'w':
            # heuristic = abs((len(maze)-2) - (curr_loc[0]-1)) + abs((len(maze[0])-2) - curr_loc[1])
        ...
        if curr_loc[1] > 1 and (left not in close_list) and obstacles[curr_loc[0]][curr_loc[1]-1] != 'w':
            # heuristic = abs((len(maze)-2) - (curr_loc[0])) + abs(len(maze[0])-2 - curr_loc[1]-1)
...
def AForwardDriver(maze):
    ...
    # huristic = [[0 for _ in range(w)] for _ in range(h)]
    for i in range(h):
        for j in range(w):
